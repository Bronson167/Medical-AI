...
# def chat():
    ...
    try:
        ...
        lang = detect(msg)
        lang_map = {"ht": "créole haïtien", "fr": "français"}
        language = lang_map.get(lang, "français")

        # 2️⃣ Charger l'historique complet
        chat_history = memory.load_memory_variables({})["chat_history"]
        all_messages = " ".join([m.content for m in chat_history])
        context = f"Historique des symptômes précédents : {all_messages}" if all_messages else "Aucun symptôme précédent."

        # 3️⃣ Préparer le prompt pour le LLM
        prompt_input = {
            "input": f"Réponds en {language}. Question: {msg}",
            "context": context,
            "language": language,
            "chat_history": chat_history
        }

        # 4️⃣ Appel au RAG + LLM Groq
        response = rag_chain.invoke(prompt_input)

        # 5️⃣ Sauvegarder le message et la réponse dans la mémoire
        memory.save_context(
            {"input": f"Question: {msg}"},
        ...
        return jsonify({"answer": f"Erreur serveur : {str(e)}"}), 500
    data = request.get_json()
    msg = data.get("msg", "").strip()
    if not msg:
        return jsonify({"error": "No message provided"}), 400
    ...
    try:
        ...
        lang_map = {"ht": "créole haïtien", "fr": "français"}
        language = lang_map.get(lang, "français")
        msg_for_llm = f"Question: {msg}"

        # =========================
        # 2️⃣ Charger l'historique
        # =========================
        chat_history = memory.load_memory_variables({})["chat_history"]
        all_messages = " ".join([m.content for m in chat_history])
        context = f"Historique des symptômes précédents : {all_messages}" if all_messages else "Aucun symptôme précédent."
        ...
        # =========================
        # 5️⃣ Sauvegarder le message et la réponse
        # =========================
        memory.save_context({"input": msg_for_llm}, {"output": response.get("answer", "")})
    ...
    try:
        ...
        # =========================
        chat_history = memory.load_memory_variables({})["chat_history"]
        # Extraire tout le texte pour créer le contexte des symptômes
        all_messages = " ".join([m.content for m in chat_history])
        context = f"Historique des symptômes précédents : {all_messages}" if all_messages else "Aucun symptôme précédent."
    ...
    try:
        ...
        lang_map = {"ht": "créole haïtien", "fr": "français"}
        language = lang_map.get(lang, "français")
        msg_for_llm = f"Réponds en {language}. Question: {msg}"

        # Charger l'historique
        chat_history = memory.load_memory_variables({})["chat_history"]
        all_messages = " ".join([m.content for m in chat_history])

        # Contexte pour le LLM médical
        context = f"Historique des symptômes précédents : {all_messages}"
        ...
        memory.save_context({"input": msg_for_llm}, {"output": response["answer"]})
        answer = response.get("answer", "Désolé, aucune réponse disponible.")
        return jsonify({"answer": answer})
    except Exception as e:
    ...
    try:
        ...
        if lang == "ht":
            ...
            msg = f"Utilisateur parle français. Réponds en français.\nQuestion: {msg}"
        else:
            msg = f"Utilisateur parle une langue non identifiée ({lang}). Réponds en français.\nQuestion: {msg}"
